      ...
      >
        ...
        <div className={Styles.projectsWrap}>
          <ul className={Styles.projects} data-ref="grid">
            ...
            {posts.map((post, index) => (
              <Thumbnail aria-hidden="true" index={index + perPage} key={post.id} post={post} />
            ))}
            {posts.map((post, index) => (
              <Thumbnail aria-hidden="true" index={index + perPage * 2} key={post.id} post={post} />
            ))}
            {posts.map((post, index) => (
              <Thumbnail aria-hidden="true" index={index + perPage * 3} key={post.id} post={post} />
            ))}
          </ul>
        </div>
      </main>
